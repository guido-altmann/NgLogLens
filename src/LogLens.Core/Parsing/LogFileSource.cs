...
public sealed class LogFileSource(string name, Func<CancellationToken, ValueTask<TextReader>> openAsync)
{
    ...
    /// <summary>Für den Upload im Browser: der Stream wird erst beim Lesen geöffnet.</summary>
    public static LogFileSource FromStream(string name, Func<CancellationToken, ValueTask<Stream>> openStreamAsync)
